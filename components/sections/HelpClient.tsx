          ...
          setLinkGroups({
            col1:
              sorted[0]?.links
                ...
                .map((l) => ({ label: l.label, href: l.href })) || fallbackLinkGroups.col1,
            col2:
              sorted[1]?.links
                .sort((a, b) => a.order - b.order)
                .map((l) => ({ label: l.label, href: l.href })) || fallbackLinkGroups.col2,
            col3:
              sorted[2]?.links
                .sort((a, b) => a.order - b.order)
                .map((l) => ({ label: l.label, href: l.href })) || fallbackLinkGroups.col3,
          })
        }
      } catch (error) {
    ...
    <>
      ...
      >
        <section className="w-full max-w-6xl px-4 md:p-8">
          <div
            className="container flex flex-col items-center p-4 mx-auto md:p-8 rounded-3xl relative"
            style={{ backgroundColor: isDark ? 'rgba(255,255,255,0.03)' : '#f3f4f6' }}
          >
            <motion.h1
              variants={item}
              className="text-3xl font-bold leading-none text-center sm:text-4xl mb-2"
            >
                      ...
                      <ul>
                        {results.map((result, idx) => (
                          <li
                            ...
                            style={{ borderColor: isDark ? '#262626' : '#f1f5f9' }}
                          >
                            <Link
                              href={result.href}
                              className="flex flex-col px-6 py-4 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
                            >
                              <span
            ...
            <motion.div
              variants={item}
              className="flex flex-col w-full divide-y sm:flex-row sm:divide-y-0 sm:divide-x sm:px-8 lg:px-12 xl:px-32 relative z-10"
              style={{ borderColor: isDark ? 'rgba(255,255,255,0.1)' : '#d1d5db' }}
            >
              <div
                className="flex flex-col w-full divide-y"
                style={{ borderColor: isDark ? 'rgba(255,255,255,0.1)' : '#d1d5db' }}
              >
                {linkGroups.col1.map((link, i) => (
                  <Link
                    key={i}
                    href={link.href}
                    className="flex items-center justify-center p-4 sm:py-8 lg:py-12 hover:bg-black/5 dark:hover:bg-white/5 transition-colors"
                  >
                    {link.label}
                  </Link>
                ))}
