...
import { normalizeIconName } from "@/lib/utils";
...
const fallbackValues: TeamValue[] = [
  { icon: "target", iconColor: "#ff6929", title: "Our Mission", description: "To empower restaurant owners with world-class technology that simplifies operations and drives growth.", order: 1 },
  { icon: "favorite", iconColor: "#ef4444", title: "Customer First", description: "We believe in building relationships, not just software. Your success is our success, and we're with you every step.", order: 2 },
  { icon: "group", iconColor: "#3b82f6", title: "Local Roots", description: "Born in Nepal, for Nepal. We understand the unique challenges of the local market and build solutions that work here.", order: 3 },
];

const fallbackData: Partial<TeamPageData> = {
  title: "Meet the Team",
  subtitle: "Yummy Ever is the flagship product of Everacy. We are a passionate group of foodies, engineers, and designers on a mission to revolutionize the restaurant industry.",
  companyHighlight: "Everacy",
  values: fallbackValues,
  joinSection: {
    title: "Want to join us?",
    description: "We are always looking for talented individuals to join our growing team. Check out our open positions.",
        ...
        <div className="max-w-6xl mx-auto text-center">
           ...
           <motion.p variants={item} className="text-xl mb-16 max-w-2xl mx-auto" style={{ color: isDark ? '#a3a3a3' : '#64748b' }}>
             {data.subtitle ? (
               <>
                 ...
                 <span className="font-bold text-orange-500">{data.companyHighlight || "Everacy"}</span>
           ...
           {loading ? (
             <motion.div variants={item} className="grid md:grid-cols-3 gap-8 text-left mb-20">
               {[1, 2, 3].map((i) => (
                 <div key={i} className="p-8 rounded-3xl border animate-pulse" style={{ borderColor: isDark ? '#262626' : '#e2e8f0', backgroundColor: isDark ? '#171717' : '#ffffff' }}>
             ...
             <motion.div variants={item} className="grid md:grid-cols-3 gap-8 text-left mb-20">
               {values.sort((a, b) => a.order - b.order).map((value, idx) => (
                 <div key={idx} className="p-8 rounded-3xl border transition-all hover:shadow-lg" style={{ borderColor: isDark ? '#262626' : '#e2e8f0', backgroundColor: isDark ? '#171717' : '#ffffff' }}>
                   <span 
                     className="material-symbols-outlined text-3xl mb-4 block"
                     style={{ color: value.iconColor }}
                   >
                     {normalizeIconName(value.icon)}
                   </span>
                   <h3 className="text-xl font-bold mb-2" style={{ color: isDark ? '#ffffff' : '#0f172a' }}>{value.title}</h3>
           ...
           <motion.div variants={item} className="rounded-3xl p-12 relative overflow-hidden" style={{ backgroundColor: isDark ? '#171717' : '#f8fafc' }}>
               <div className="relative z-10">
                   <h2 className="text-3xl font-bold mb-4" style={{ color: isDark ? '#ffffff' : '#0f172a' }}>{joinSection.title}</h2>
                   <p className="mb-8 max-w-xl mx-auto" style={{ color: isDark ? '#a3a3a3' : '#64748b' }}>
                       {joinSection.description}
                   </p>
                   <a href={joinSection.ctaHref} className="inline-block px-8 py-3 rounded-full bg-primary text-white font-bold hover:bg-orange-600 transition-colors">
                       {joinSection.ctaText}
                   </a>
               </div>
           </motion.div>
